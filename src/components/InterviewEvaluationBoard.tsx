...
export function InterviewEvaluationBoard({
  editableSubdomains,
  initialSubdomain,
  initialCriteria,
  initialApplications,
  ...
  const [criteriaDraft, setCriteriaDraft] = useState<InterviewCriterion[]>(
  ...
  async function switchSubdomain(next: string) {
    ...
    try {
      const res = await fetch(`/api/interview-board?subdomain=${encodeURIComponent(value)}`);
  ...
  async function toggleAttendance(applicationId: string) {
    ...
    try {
      const res = await fetch(`/api/applications/${applicationId}/interview-score`, {
        method: "PATCH",
  ...
  async function saveRow(applicationId: string) {
    ...
    const rowScores: InterviewCriterionScore[] = criteria
      .map((c) => {
        const score = getScore(applicationId, c.id);
        return score !== undefined ? { criterionId: c.id, score } : null;
      })
      .filter((s): s is InterviewCriterionScore => s !== null);
    try {
      const res = await fetch(`/api/applications/${applicationId}/interview-score`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveCriteria() {
    ...
    try {
      const res = await fetch("/api/interview-criteria", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 text-xs text-on-surface-variant">
                              ...
                              {a.status !== "INTERVIEW" && <StatusBadge status={a.status} />}
                            </div>
                          </div>
