  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold text-on-surface">{app.name}</h1>
      ...
      {evaluation && (
        <Card className="border-primary/30 bg-gradient-to-r from-primary/10 to-transparent">
          <CardContent className="p-6">
            ...
            <p className="text-sm leading-relaxed text-on-surface">{evaluation.recommendation}</p>
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            {app.githubUsername && (
              ...
              >
                <GitBranch className="h-3.5 w-3.5" /> {app.githubUsername}
              </a>
            )}
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          {questions.map((q) => (
            <div key={q.id}>
              <p className="text-sm font-medium text-on-surface-variant">{q.label}</p>
              <p className="mt-1 whitespace-pre-wrap text-sm text-on-surface">
                {app.questionnaire[q.id]?.trim() || <span className="italic text-on-surface-variant">No answer</span>}
              </p>
            </div>
          ))}
        </CardContent>
