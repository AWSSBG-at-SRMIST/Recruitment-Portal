...
import { canEditGDCriteria, canViewApplication } from "@/lib/permissions";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const score = await repo.getGDScore(id);
  return NextResponse.json({ score });
}

function isValidScores(value: unknown): value is GDCriterionScore[] {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const currentCriteria = await repo.getGDCriteria(application.subdomain);
